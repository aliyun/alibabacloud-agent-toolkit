...
| `SystemDisk.*` — **flat, not nested** | `--system-disk-category cloud_essd` · `--system-disk-size 40` · `--system-disk-performance-level PL0` | ❌ dot form `--system-disk.category` → `unknown flag`. ❌ space-separated nested form `--system-disk Category=cloud_essd Size=40` — that shape belongs to `--data-disk`, NOT to the system disk. Verified against `ecs describe-price --help` on CLI 3.5.0, 2026-09-11: the official example passes `--system-disk-category cloud_ssd --system-disk-size 80 --system-disk-performance-level PL1` as three flat flags, while data disks use `--data-disk Category=cloud_ssd Size=2000 PerformanceLevel=PL1` |
| `SystemDisk.PerformanceLevel` — **omitting it costs money** | always pass it explicitly for `cloud_essd`: `--system-disk-performance-level PL0` | The API default is **PL1**, not PL0. Measured 2026-09-11 (`ecs describe-price`, `ecs.c9i.large`, 40G `cloud_essd`, cn-beijing, Month): **PL0 → ¥205.91**, **PL1 → ¥225.91**, **flag omitted → ¥225.91**. So a query that drops the flag quotes ¥20/month above the price of the machine that will actually be created, breaking the pricing contract in the SKU matrix. Not valid for `cloud_essd_entry` (starter) — omit it there |
| Region — **`ecs describe-price`** | **BOTH: the global `--region cn-beijing` AND `--biz-region-id cn-beijing`** | `--biz-region-id` alone reaches only the request body; the **endpoint** still comes from the profile's default region. Measured in eval run 8626: `--biz-region-id cn-beijing` on a cn-hangzhou-default profile returned `HostId: ecs-cn-hangzhou.aliyuncs.com` and failed `InvalidSystemDiskCategory.ValueNotSupported`. `--help` states `--region` is the global flag used for signing and endpoint resolution, and the official example passes both. Same trap as `swas-open` above |
| Nested object params | `--data-disk Category=cloud_essd Size=40` | dot form `--data-disk.category` → `unknown flag` |

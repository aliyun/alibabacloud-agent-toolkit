...
Step -1.5.0: Read the capability matrix — executable prerequisite
  Use the `Read` tool to open `references/cli_capability_matrix.md` before issuing any Alibaba Cloud CLI command.
  A directory listing that merely shows the filename does NOT count. Recalling the table from memory does NOT count.
  A live Describe/List call does NOT substitute for this static lookup; those calls belong to Step 0.2b only.
  If the file cannot be read, STOP. Do not claim CLI reachability and do not begin the permission checks.
...
Step -1.5.3: Any partial/false hit → NAME it in the summary as a self-open step (announce-only, do NOT stop and wait)
  ...
  ⚠️ **Announce-only — these manual/console products are NOT a gate at -1.5.** They are self-purchase add-ons, not preconditions for the CLI-reachable resources, so naming them here is enough: do NOT hand the flow back and wait for the user to go buy/open them, and do NOT say 「开完回来我再继续」. Keep going with everything you can automate; the "walk you through opening it" plus any real blocking point live at the LATER step that depends on it (OSS package → `confirm-authorize.md` Step 1.3 + the bucket step in `provision.md`; Token Plan / PDS → their yaml steps in Phase 3). Pushing the user to buy something at -1.5 sends them off before they have seen the plan or the price — measured: a weak model ended -1.5 with 「你先去点开链接…开完回来…我接着帮你创建剩下的资源」 and stalled the whole deployment on optional add-ons.
  User-facing copy template (friendly + plain metaphors, no technical jargon) — announce it, then continue:
    "你这个套餐里有 ${产品比喻名}（${正式名}），这部分没法用命令行一键开、需要你自己在页面上开一下：
     ${控制台/分享链接}
     这个等会儿到那一步我再提醒你，现在我先把能自动开的都准备好。"
  ...
  Typical fallbacks:
    ...
    - OSS storage package (partial) → https://common-buy.aliyun.com/?commodityCode=ossbag&regionId=china-common
      ⚠️ ANNOUNCE ONLY HERE — like every manual item at -1.5, this is announce-only and does NOT stop and wait.
      Naming it in the reachability summary is enough; the blocking confirmation belongs to Step 1.3 in
      ...
      bucket call — an unactivated account FAILS even the read-only probe (`ossutil ls` → 403 UserDisable,
      EC 0003-00000801, E2E-measured 2026-09-16), so the Step 0.2b probe DOES catch it; score that code as
      "not activated" and route to this announce-only handover, not as a permission gap.

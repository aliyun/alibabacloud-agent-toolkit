...
8. **CLI runs through MCP `CallCLI`**: every `aliyun` command goes out via the MCP tool `AlibabaCloud___CallCLI` (sandboxed execution, auditable agent identity, RAM identity downgrade), with the command passed through **exactly as written** — same product, same kebab-case action, same flags. It is a transport, not a different API surface: never rewrite a command into PascalCase API form, never invent parameter names, and never substitute a metadata tool (`ListApis`, `GetApiDefinition`, `SearchApis`, …) for an execution a command already covers. If the MCP server is unavailable in the current runtime, fall back to running the same command directly in Bash. The channel is internal — never surface `CallCLI`, MCP, or the fallback to the user.
...
18. **Inbound exposure is opened per port, per source — never with a wildcard where a single address belongs.** Two hard cases:
    - **SSH port 22 must be tightened to `${MY_IP}/32`**: when creating/reusing a security group, the SSH port-22 rule's `SourceCidrIp` must not be `0.0.0.0/0`. The skill automatically runs `curl -4 -s --max-time 5 https://ifconfig.me` to get the egress IPv4 and writes it into the rule (`-4` forces IPv4 — security-group SourceCidrIp does not accept IPv6). When blocked by an org ControlPolicy, **do not teach the user to bypass it**; guide them to contact the admin to request an exception or widen the sub-account permissions. Keep the rule after deployment (the user can tighten/close it in the console themselves).
    - **The database whitelist must be the ECS private IP `/32`**: every RDS create step carries `SecurityIPList` in its yaml (`${ecs_private_ip}/32`). Passing `0.0.0.0/0` instead is FORBIDDEN — measured, a run did exactly that, which opens the instance to every client able to reach the intranet and breaks the SKU file's own promise to the user (`RDS SecurityIPList only lists the ECS intranet IP (no public access)`). No public network type either: `DBInstanceNetType` stays `Intranet`. If the private IP is not resolved yet, read it from the ECS create response — a wildcard is never the way to keep the flow moving. Only HTTP/HTTPS (80/443) are legitimately `0.0.0.0/0`, and that exposure is disclosed in the payment prompt.
...
25. **SKU product CLI-reachability static gate** (concentric with iron-rule #27): after receiving the advisor SKU, **first** check the CLI capability gating matrix (cli_capability_matrix) — each product is marked `cli_supported: true | partial | false` + `fallback_route`. If any item is marked `partial` or `false`, immediately explain to the user and NAME the corresponding fallback route for it (e.g., Token Plan false → share link; PDS partial → primary-account enablement page; the OSS package partial → buy page), and **forbid CLI trial-and-error**. This is **announce-only, NOT a stop-and-wait handoff**: these console-only / self-purchase add-ons are not preconditions for the CLI-reachable resources, so keep provisioning what you can automate and do NOT suspend the flow to wait for the user to open them — the real blocking point (if any) lives at the later step that depends on it, never at -1.5. **ESA is CLI-provisioned for every tier** — do not announce it as a manual item. This Phase -1.5 static gate happens **before** the Phase 0 credential config, to avoid the user finding out the SKU is unworkable only after configuring credentials.

...
8. The role's trust policy carries **no `acs:MFAPresent` condition**, and the setup flow does not ask the user to bind MFA. Two reasons, both measured: `opc-deploy` is created with API access only (never console access), so user-level MFA guards a sign-in path that does not exist for it; and requiring MFA on AssumeRole would break the skill outright, because an MFA code is a TOTP that rotates within the minute and cannot be stored in a `RamRoleArn` profile — `aliyun configure` has no field for it and CLI 3.5.0 rejects `--serial-number` as an unknown flag. The protection against a leaked long-lived AK comes from the sub-account holding `AliyunSTSAssumeRoleAccess` alone: the key by itself can call no business API, and the token it can obtain expires in an hour.

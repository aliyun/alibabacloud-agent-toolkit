...
- ⚠️ **These phase numbers/labels (`-2` / `-1` / `-1.5` / `0` / `0.2b` / `0.5` / `0.4` / `1`…`4`) are an INTERNAL index only — never shown to the user.** Not in chat, not in a plan / to-do / 计划表, not in files. The user-facing wording for each step is the `Plan item to write` column in deploy `SKILL.md`; when you present a plan or update the to-do list, copy those plain-Chinese items with **no** phase number or `Phase`/`阶段` label attached (SKILL.md Hard Gate #6).

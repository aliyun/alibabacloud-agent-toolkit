...
`not authorized` anywhere in the response. Codes actually observed:
  ...
  oss    → aliyun ossutil ls --profile <p>
           ...
           # ⚠️ This probe is DOUBLE-DUTY — it proves permissions AND tells OSS activation apart.
           #    E2E-measured 2026-09-16 on both states: on an ACTIVATED account `ossutil ls` →
           #    "Bucket Number is: N" and `ossutil api list-buckets` → full XML; on an UNACTIVATED
           #    account the very same reads FAIL 403 UserDisable (EC 0003-00000801). Score UserDisable
           #    as "not activated" → the Phase -1.5 announce-only handover, never a permission failure.
...
it sends the user in circles: they swap the policy and it still fails. **At this probe step (0.2b), `UserDisable` is announce-only — do NOT stop and wait for activation here.** Note it once as a self-activation item (buying the storage package activates it), keep probing the remaining products, and continue the flow. `state.self_purchase.oss_package = deferred` PASSES Step 1.5 item 10, and the ONLY real block is the bucket step (`provision.md` Pass 3c), which happens after payment — a run that declines at the payment gate never even reaches it. Ending the turn with 「激活好后告诉我，我继续」 at this probe is the exact stall to avoid (same principle as the Phase -1.5 announce-only rule: a user-self-open item is never the agent's blocking point). Never fold it in with the 403 permission cases above.

...
fallback_note: |
  「小店面」（ECS 云服务器 经济型e）¥99/年如果没命中（比如你之前已经享过同类优惠），
...
【处方】
→ **starter_webui**（「小店面」（ECS 云服务器 经济型e）¥99/年活动价，约 ¥8.25/月）
...
为什么是它：
- 介绍页不需要 AI 调用——不多花这笔钱
- 「小店面」（ECS 云服务器 经济型e）¥99/年长期优惠（续费同价），最经济的入门选择
- 你已经有 Codex，代码部署靠它远程帮你搞定就行

【你会拿到什么】
- 你的「小店面」（ECS 云服务器 2核2G）：网页跑在上面，邻居随时能打开
- 「全球加速」（ESA 边缘安全加速 免费版）：访问速度快
...
【什么时候该升级】
- 想加下单/付款功能（需要后端 + 数据库）→ 升 lite_seed（¥596.54/月，去「小店面」（ECS 云服务器）/「客户档案柜」（RDS 数据库）控制台升配，或跟我说一声我帮你判断）
...
```yaml
sku: starter_app
price: ¥212.88/月（北京，含「小店面」（ECS 云服务器 经济型e）¥99/年摊月 ¥8.25 + Token+PDS+ESA ¥204.63/月）
disclaimer: "💡 价格供参考，实际以最终下单为准。"
purchase_action: |
  接下来你可以：
  A. 让我帮你下单 —— 你说一声，我直接帮你创建好（推荐）
...
【处方】
→ **starter_app 档**（¥204.63/月 + 「小店面」（ECS 云服务器 经济型e）¥99/年摊月 ¥8.25 ≈ **¥212.88/月**，北京地域）
...
- 「业务专用店面」（ECS 云服务器 2核4G）：小程序跑在这里
- 「客户档案柜」（RDS 数据库 1核2G · 100GB）：用户账号 + 记账数据安全存储
...
- 几百付费用户的课程数据需要独立可靠的数据库（RDS 数据库 2核4G）
- 业务跟 AI 助理分开跑，互不干扰

【你会拿到什么（大白话清单）】
- 「AI 助理的家」（轻量应用服务器 · OpenClaw 2核4G）：AI 助理帮你运维
- 「AI 店员工时包」（Token Plan 标准版 25k/月）
- 「业务专用店面」（ECS 云服务器 2核4G）：小程序跑在这里，承接三四十人同时访问
- 「客户档案柜」（RDS 数据库 2核4G · 100GB 数据存储空间）：用户数据 + 课程订单安全存储
- 「储物间」（OSS 对象存储 40GB + 阿里云盘 200GB）：课程视频和资料
- 「全球加速」（ESA 边缘安全加速 免费版）
...
- **HTTPS 强制开启**：所有页面走 https，避免用户密码 / 订单数据被中间人窃听（ESA 边缘安全加速 免费证书自动续签，到期前 30 天自动续，你不用管）
...
purchase_action: |
  ...
  B. 自己去页面买 —— 打开 https://opc.aliyun.com/products
     → 选「OPC Pro 套餐」卡片
     → 保持默认配置（ECS 云服务器 4核8G × 2，对应 pro_steady）
     → 确认付款
...
- 「AI 店员工时包」（Token Plan 标准版 25k/月）
- 「业务专用店面 × 2」（ECS 云服务器 4核8G × 2 台）：一台挂了另一台顶着
- 「客户档案柜·高可用版」（RDS 数据库 高可用版 4核8G · 200GB 数据存储空间）：主备自动切换
- 「流量调度员」（ALB 应用型负载均衡）：自动分发请求

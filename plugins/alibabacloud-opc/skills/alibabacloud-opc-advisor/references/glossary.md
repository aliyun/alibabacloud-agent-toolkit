...
「业务专用店面」 (dedicated business storefront) = ECS Cloud Server (Lite/Pro)
  → 「业务专用店面」（ECS 云服务器 XX核XXG）——跟 AI 助理物理分开，专门承接真实流量
...
- ECS row: **`「业务专用店面」（ECS 云服务器 XX核XXG）`** — physically a separate machine from the SWAS where the AI assistant lives

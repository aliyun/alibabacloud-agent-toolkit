...
Practical details for two environmental questions around generated Terraform:
**how the alicloud provider finds credentials** and **what users can do later
when their own local `terraform init` cannot reach the upstream registry**.
The skill itself validates through MCP/IaCService and does not run local
Terraform.
...
### Optional deployment probe (non-reading)

If a user later asks whether their environment is ready for local Terraform
plan/apply, probe **presence** of each mechanism without reading any value. Any
one line of output = ready:
...
`~/.terraformrc` is a **user-global** configuration file. If the user later
runs local Terraform and hits a registry network failure, the agent MUST:

...
triggers: >
  aliyun CLI, Aliyun CLI, CLI command, CLI command, aliyun command, command-line tool,
  CLI plugin, plugin install, CLI install, CLI configuration, CLI error, CLI error,
  InvalidAccessKeyId, SignatureDoesNotMatch, Throttling, biz-region-id,
  kebab-case, plugin command, CLI syntax, command syntax, aliyun plugin,
  generate CLI command, generate CLI command, CLI parameter, CLI parameter
license: Apache-2.0
metadata:
  domain: aliyun-cli
  owner: sdk-team

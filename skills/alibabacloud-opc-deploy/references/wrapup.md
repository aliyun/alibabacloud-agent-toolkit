...
Step 4.3.5: When to scale up (relocated from the advisor prescription — the advisor now only points here; show once, pick the tier matching the SKU just deployed)
  Preface: "现在这套配置够用了。等业务长大，下面是升级路线——升档前我都会先报出新的月费，你确认了才执行："
  Starter (starter_webui / starter_app):
    "- 有了持续用户量（几十人）+ 需要独立数据库 → 升 lite_seed（¥596.54/月）
     - 想加 AI 功能 → 切 starter_app（+¥204.63/月，多了 Token Plan + 云盘）
     - Token Plan 25k/月 不够 → 控制台一键升高级版（100k，¥698/月）"
  Lite (lite_seed / lite_growth / lite_traction):
    "- 「客户档案柜」（RDS 数据库）CPU 持续 >70% → 升 lite_growth（¥736.54/月）
     - 「业务专用店面」（ECS 云服务器）也吃紧 / 稳定到一两百人 → 升 lite_traction（¥1131.45/月）
     - 持续超两百人 + 不能停机 → 升 pro_steady（¥2524.55/月，多机 + 高可用）
     - 升配不停机（如 2核4G→4核8G），热迁移支持"
  Pro (pro_steady / pro_burst):
    "- 出现突发尖刺（直播/活动）→ 升 pro_burst（加弹性伸缩）
     - 持续超千人 / 多地域 / 合规要求 → 联系阿里云销售定制方案
     - Token Plan 25k 不够 → 一键升高级/尊享版"

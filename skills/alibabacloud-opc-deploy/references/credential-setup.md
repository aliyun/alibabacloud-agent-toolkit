...
Step 0.2: Detect an already-configured credential (read-only; never touches the secret)
  ...
  b) Resolve the deploy profile (the "opc profile" referenced throughout this skill):
     ...
     ⚠️ Pin ONE profile and use it for the whole deployment. Do NOT silently fall back to a second profile
       ...
       → From here on, that name is `${PROFILE}`: every command template in this skill writes
         `--profile ${PROFILE}` so the templates stay copy-paste runnable in ANY environment. Bind it in the
         same shell call that runs the command (`PROFILE=<resolved name>; aliyun …`) — each call is a fresh
         shell — or substitute the name into the flag. Substitute ${PROFILE} and nothing else.
  Outcomes:
  ...
  ⚠️ Red lines unchanged: this step NEVER reads/prints/echoes AK/SK, NEVER runs `aliyun configure set` with literal values, NEVER asks the user to paste AK/SK. **Detect ONLY via `aliyun configure list` (masked) + `aliyun sts get-caller-identity` (identity only); NEVER `cat` / `less` / `grep` / open `~/.aliyun/config.json` to inspect a credential — that file holds the plaintext AK/SK + StsToken, and reading it prints the secret straight into the tool log (iron-rule #15, a red-line violation).** It only *uses* an already-configured profile via the CLI's own signing — the skill never sees the secret.

  🔒 **Why interactive `aliyun configure` is safe, stated explicitly.** In Step 0.3 the user types their

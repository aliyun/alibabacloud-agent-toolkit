...
- ✗ `进入 Phase 1：询价` → ✓ `正在查询各项的实时价格`
This is a **preventive** rule, not a post-hoc self-check: phrase it right the first time. If a label slips into something already sent, restate that part in plain Chinese immediately.

## CLI invocation channel

Every `aliyun` command in this skill and its references runs through the MCP tool `AlibabaCloud___CallCLI`, provided by the `alibabacloud-opc` MCP server. It executes remotely in a sandbox under an auditable agent identity and supports RAM identity downgrade, so a command sent this way cannot act beyond the role it was granted.

`CallCLI` is a transport, not a different API surface. **Pass every command through exactly as written here** — same product, same kebab-case action, same flags. Never rewrite a command into PascalCase API form, never invent parameter names for it, and never substitute a metadata tool (`ListApis`, `GetApiDefinition`, `SearchApis`, …) for an execution that a command already covers.

If the MCP server is not available in the current runtime, run the same `aliyun` command directly in Bash instead. The channel is internal mechanics: never mention `CallCLI`, MCP, or the fallback to the user.
...
See `references/execution-phases.md` — it is a short index; each phase's real steps live in its own file, named in the table below. **Read the phase's file when you enter that phase; never execute a phase from this summary alone.** For Phase -1.5, this means an explicit `Read` of `references/cli_capability_matrix.md` before the first Alibaba Cloud CLI call: listing the directory, remembering the table, or replacing the lookup with a live Describe/List request does not satisfy the gate.

⚠️ **Phase numbers/names are an INTERNAL index — never shown to the user; the canonical phase ordering lives only in `references/execution-phases.md`.** The user's plan / to-do / progress line is built **only** from the `Plan item to write` column below (already plain Chinese, no internal labels), one item per line. When you show the user a plan or update the to-do list, copy those items verbatim — do **NOT** prefix them with a phase number or the word `Phase`/`阶段`/`第N阶段`, do **NOT** paste the `Name`/`File`/`Summary` columns, and do **NOT** reproduce this table's grid. Rendering a plan line shaped like `Phase -1：确认账号...` / `阶段0：...`, or announcing `进入 Phase 0.4` / `Step 0.2b 权限探测`, is a Hard Gate #6 violation (measured: a weak model reproduced this table across three plan updates and leaked `Phase -1`…`Phase 4` 22 times). Say only the plan item, e.g. `确定操作系统镜像和机器配置`.

| Plan item to write (copy verbatim — the ONLY column the user may see) | Name (internal) | File | Summary (internal) |
|------|------|------|------|
| `先把要开哪个套餐定下来` | Settle the SKU (**runs first**) | `sku-resolution.md` | Recognize one of the 7 exact SKU tokens; if none, run the Advisor bootstrap block and STOP. Nothing downstream works without a settled SKU. |
| `确认账号和实名，看看我能不能帮你部署代码` | Pre-checks | `preflight.md` | Confirm Alibaba Cloud account + real-name verification + take over code-deployment capability |
| `先查清这个套餐用到的产品哪些我能自动开、哪些要你自己开` | CLI reachability gate (**MANDATORY standalone** — run before Phase 0; do NOT merge into Phase 0/2 or skip) | `preflight.md` | For every product this SKU touches, determine CLI reachability by **STATIC table lookup in `references/cli_capability_matrix.md` ONLY — do NOT run any aliyun CLI command to probe/verify reachability at this phase (iron-rule #25; the first real CLI call is in Phase 0)**. On any console-only product (e.g. Token Plan = console-only, no CLI path), your user-facing summary MUST explicitly name it as a step the user opens themselves later — never omit it or list only the CLI-reachable products. This is **announce-only, NOT a stop-and-wait**: a console-only / self-purchase add-on is not a precondition for the CLI-reachable resources, so name it and **keep provisioning what you can automate** — do NOT hand the flow back and wait for the user to go buy/open it at this phase (that premature handoff stalls the whole deployment on optional add-ons). Keep the resulting product list: Step 0.2b and the RAM policy are both scoped to it. |
| `检查命令行工具，需要就自动装好` | CLI install | `cli-install.md` | Fully automated install/upgrade + `aliyun configure set --auto-plugin-install true` (required before ANY product command in CLI 3.4.x) |
| `检查现有凭证，没有就带你配一套专用的` | Credentials | `credential-setup.md` | Detect an existing usable credential and USE it, else configure RamRoleArn. Pin ONE profile for the whole run — different profiles may belong to different accounts. AK/SK never enter the conversation. |
| `先试一下权限够不够（只读、不花钱）` | **Policy coverage probe (🔒 HARD GATE — before ANY paid call)** | **`policy-probe.md`** | **Run one read-only Describe/List per product in THIS SKU's product set. Zero cost, creates nothing. Any 403 / NoPermission / Forbidden.RAM ⇒ STOP, do NOT enter Phase 1/2/3, and hand the user a whole-policy replacement JSON scoped to this SKU (never an incremental patch, never the all-SKU superset). Skipping this probe means discovering the missing permission mid-Phase-3, after money is already spent.** |
| `试一下能不能正常连上阿里云` | Connectivity | `credential-setup.md` | `describe-regions` succeeds, proving AssumeRole + role trust + policy are correct |
| `确定操作系统镜像和机器配置` | Image resolution | `image-resolution.md` | Centrally resolved and permanently bound in state (exact ImageFamily from advisor, else default Linux 3 2104 LTS; Linux 4 is experimental and forbidden for production) |
| `实时询价，把资源清单和月费算给你确认，等你点头` | Confirm + authorize | `confirm-authorize.md` | Price inquiry (starter dual-tier) + resource list + component-removal opt-out + second payment confirmation + pre-execution hard-gate self-check. SKU is already settled — do not re-identify it here. |
| `搭网络（顺手把远程登录只对你自己的 IP 开放）` | Network infra | `network.md` | VPC (reuse-first) + VSwitch + security group. Whether you CREATE or REUSE the security group, you MUST call `authorize-security-group` to tighten SSH (port 22) to `${MY_IP}/32` for the current user — never skip it on reuse or assume an existing group's rules are correct (HTTP 80 / HTTPS 443 stay 0.0.0.0/0). |
| `按顺序开通资源，每扣一笔都告诉你` | Create resources (💰 paid) | `provision.md` | Execute in order per `references/sku-params/<sku>.yaml` steps. AFTER EACH create call: (a) call TagResources to attach `opc:managed=true` + `opc:sku=<sku>`; (b) write the resource ID + status to the state file. Both are mandatory before proceeding to the next step. |
| `收尾：交付信息、月费合计、续费提醒、出事怎么办` | Verify + wrap-up | `wrapup.md` | Status verification + summary card + "what's next" + renewal reminder + emergency three-step incident card + state save + wrap-up completeness hard-gate. Deployment-failure teardown lives here too. |

> The phase numbers and canonical ordering behind these steps are internal-only — they live in `references/execution-phases.md` and must never appear in anything the user sees (plan, to-do, chat, or files).
...
1. **PAYMENT GATE (HARD BLOCK — enforced for weak and strong models alike)**: Immediately BEFORE the first fee-incurring API call (RunInstances / CreateInstance / swas-open CreateInstances / PurchaseRatePlan / CreateDBInstance / CreateLoadBalancer / CreateScalingGroup — anything that spends money), run a literal self-interception: since showing the resource list, have I (a) output the EXACT prompt below verbatim, AND (b) received an explicit affirmative reply that authorizes the CHARGE? If NO → **STOP, DISCARD the pending create call, output the exact prompt below verbatim, and wait for the reply. The prompt MUST be the LAST thing you output this turn — after emitting it, issue NO further tool call (no network, no create, nothing) and END your turn to yield to the user.** An earlier deploy-intent reply (a "let's deploy / start creating / confirm purchase"-style message given BEFORE this prompt) is INTENT only and does NOT authorize the charge — never treat it as payment authorization. **The user's OPENING request is the clearest case of this**: `帮我开一个 lite_seed` / `帮我部署 starter_webui` / `把我的站点弄上线` is the instruction that STARTS the flow, and it can never be the answer to a prompt it predates. Self-talk of the shape 「由于你已明确要求"帮我开一个 …"，我将继续执行」 is this violation with a justification attached — measured, one run reasoned exactly that, skipped the prompt entirely and created SWAS + ECS + RDS for ¥391.91/month that the user never approved. The only thing that authorizes a charge is a user turn that lands AFTER the prompt. **Amount rule**: ¥XX MUST be the actual price from Phase 1's inquiry — the promo price when the ¥99/yr promo is hit, or the fallback price (≈¥284.99/yr) when it is missed; never a placeholder or estimate. **Every amount you display must be traceable to a `DescribePrice`-family response you received in THIS session** — if you cannot point at the response field it came from, you may not show it and you may not create that product; a number you filled in to keep moving is a fabricated price (measured: a run showed ECS `¥783.64/月` and a `¥969.64/月` total after its inquiry errored out, said `稍后在创建时确认`, and charged ¥391.91 instead). No price → no authorization request → no create: fix the inquiry, or tell the user plainly that pricing failed and stop. Keep the fenced prompt below as the ONLY thing shown to the user here — do NOT append internal matching criteria to the user-facing prompt. Output it verbatim (fill ¥XX with that real amount):
...
The first line of the prompt above discloses that the website's 80/443 ports will be open to the public internet — it rides in the same block so the single confirmation covers both the charge and the public exposure (required by security review; omit it only when the deploy opens no public ports). Free resources (VPC / VSwitch / SecurityGroup / KeyPair) do NOT need this gate; only the first PAID call does. This is a hard gate, not a suggestion — skipping it, or paraphrasing the prompt instead of outputting it verbatim, is a critical violation. **Emit-then-yield (weak and strong models alike):** emitting the prompt and then continuing to act in the SAME turn — issuing any create / purchase / order call before the user has actually replied — is EQUALLY a critical violation; a strong model has done exactly this (it displayed the correct ¥ total, then created the first paid resource without waiting). The paid create happens ONLY in a LATER turn, after an explicit affirmative reply lands. **The mirror-image failure — stopping BEFORE the prompt — is just as disqualifying (measured on a weak model):** ending your turn after the itemized list / total / disclaimer but WITHOUT the verbatim `确认付款？` question means you showed prices and then vanished — the gate never opened, the user was never asked, and the run ended right at `💡 价格供参考，实际以最终下单为准。` with no chance to confirm OR decline. A `价格供参考`-style disclaimer is never your closing line; the `确认付款？` question is. Over-run creates without asking; under-run asks nothing at all — both skip this gate. So the question is BOTH mandatory AND the last thing you emit this turn: never drop it, never let a footer follow it.
   ...
   **Multi-order SKUs (Lite / Pro): one authorization, N charges — disclose it.** Only starter is a single paid call. Lite/Pro spend money on several products (SWAS / ECS / RDS / OSS …), and **each one is a separate Alibaba Cloud order charged as its own step**, not one bundled transaction. The single gate above still governs, but it MUST be honest about the shape of the spend:
   ...
   - **Show the arithmetic, then compare it to the displayed total — as two explicit outputs, not as an assertion.** Before emitting the payment prompt, write the addition formula in plain text using only this session's returned values (e.g. `SWAS ¥<live> + ECS ¥<live> + RDS ¥<live> + ESA ¥<live> = ¥<live total>`) and use the same total in the prompt; only proceed when they are equal to the cent. If they don't match, DO NOT paper over it by adjusting the total in your head — re-inquire each product via its own pricing API and rebuild the itemized list from scratch. The arithmetic must be an emitted output, not an internal invariant.
   ...
   **The resource list and the itemized amounts live IN THE CONVERSATION — a file is never a substitute.** The whole point of this gate is that the user reads what they are about to be charged for and then authorizes it. A reply that says `我已生成《资源清单与月费-xxx.md》，请查阅后确认` moves the disclosure out of the user's line of sight and turns the authorization into a blind one — the gate is formally "satisfied" while the user has seen nothing.
   ...
   - **Executable form of this rule, checked before the first paid call**: can I quote the itemized lines, the formula, the total AND the prompt out of a message I actually sent in this conversation? If the only place any of them exists is a file I wrote, then this gate has not happened — re-emit them in chat and end the turn. Measured: a run announced 「现在向你展示完整的资源清单和费用，确认后我开始创建资源」, wrote the list into `outputs/resource-list-lite_seed.md`, emitted no prompt, and went straight to creating. Writing the file is not the failure; treating it as the showing is.
...
6. **OUTPUT GATE**: language and terminology drift is random, so a passive "output in Chinese" instruction does not hold. Every message you send the user is in scope; item (c) below says which moments you must never skip the check at. Run this two-item check on the drafted text and rewrite it if either item trips:
   ...
   - **(b) Terminology** — does the text contain any of these internal labels? `Phase` / `阶段` / `第N阶段` / `Preflight` / `Gate` / `HARD BLOCK` / `PAYMENT GATE` / `SKU GATE` / `PROBE GATE` / `OUTPUT GATE` / `iron-rule` / `Rule #<n>` / `self-interception` / `critical violation` / `probe` / `探针` / `探测` / `RamRoleArn` / `ChainableRamRoleArn` / `AssumeRole` / `StsToken` / `STS` / `三步法` / `bootstrap` / `三态` / `state file` / `yaml`. If yes → replace with the plain-language equivalent. Mapping: `Phase 2` / `阶段2` / `第2阶段` → `搭网络这一步` (a plan / to-do item must carry NO phase label at all — drop the label, keep only the plain item); `probe / 探针 / 探测` → `先试一下权限够不够`; `RamRoleArn / AssumeRole / STS` → `临时授权`; `ChainableRamRoleArn` → `会自动续期的授权方式`; `StsToken` → `临时凭证`; `state file` → `部署记录`. The only exception is a string the user must literally type or click (a command template, a console link) — keep that verbatim, and explain it in Chinese around it.
   ...
   **Only one surface is machine-facing**: the `description` argument of a tool call (it sits in the same call object as `command`, which necessarily carries English and technical identifiers such as `stat -c "%a %n"`) — keep it tidy, but it is not the channel the user reads in order to decide. **Everything else the user can see is IN scope — and that explicitly includes the to-do / progress panel AND any 计划表 / plan you render in chat prose.** The user sees your TodoWrite panel and any plan you print, so both must pass (a) and (b): build every plan / to-do line from the plain-Chinese `Plan item to write` cell **only**, one item per line, with **no** `Phase`/`阶段`/`第N阶段`/step-number prefix and **without** reproducing the `Name`/`File`/`Summary` columns or the table grid. Measured: a weak model reproduced the whole phase table to the user across three plan updates and leaked `Phase -1`…`Phase 4` 22 times — exactly because an earlier version of this line told it the plan/to-do surface was out of scope.

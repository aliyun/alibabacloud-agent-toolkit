...
allowed-tools: "Read,Skill,WebFetch,mcp__plugin_alibabacloud-opc_alibabacloud-opc__AlibabaCloud___CallCLI"
---
...
2. If Q-userdata=Yes (Q2a or Q2b), the complete 38-item UGC hardening checklist is referenced in full — not an inline 5-8 item simplification, not a link-only reference (iron-rule #33). **"In full" means all 38 items are written out as individual lines.** Counting them instead of listing them does NOT satisfy this: a summary such as `完整 38 项涵盖：身份认证（7项）、上传内容安全（7项）…` is a forbidden substitute, and so is labelling a partial list as a `子集` / `上线前必做子集` / `摘要`. **HARD BLOCK:** count the lines you actually emitted — if the number is not 38, DISCARD the draft and regenerate with every item present. Never point the user at a reference filename to make up the difference (see item 6: filenames are internal).
...
4. **No bare acronyms in PROSE — the metaphor mapping table is the one sanctioned exception** — scan every prose sentence / bullet / section body for `ECS` / `RDS` / `OSS` / `ALB` / `ESA` / `SLB` / `PDS` / `VPC` / `CDN` / `c9i` / `HA` / `LCU` used in isolation. Every such mention must use the `「plain-language role」（<ACRONYM> <Chinese product name>[ <spec>]）` format per iron-rule #1 — ✅ `「流量调度员」（ALB 应用型负载均衡）`, `「客户档案柜·高可用版」（RDS 数据库 高可用版）`, `「业务专用店面」（ECS 云服务器 2核4G）`; ❌ `「流量调度员」（ALB）`, `「客户档案柜」（RDS）` — an acronym alone inside the parentheses is STILL a bare acronym and fails this check. **The metaphor↔official-name mapping table is the legend and is EXEMPT from this scan** — its entire purpose is to list the official product names against their metaphors, so bare product names in the mapping table's own cells are correct, NOT a violation. The table being present is NOT a licence to use bare acronyms elsewhere — the prose in `处方` / `为什么是它` / `怎么搞起来` / `怎么迁过来` must still use the inline format. **HARD BLOCK:** if ANY bare acronym survives the scan **in prose**, DISCARD the entire draft and regenerate; the inline substitution MUST be applied in every sentence (the legend table does not substitute for it).
...
8. **Deliver the whole prescription as ONE message (HARD BLOCK), and the A/B dual-path closing is the LITERAL LAST block** — every part ships together in a single reply, in this order: the five sections, the `为什么是它` rationale, the UGC checklist when Q-userdata=Yes, the metaphor mapping table, the account-ownership footnote (when Q5 unstated), and — **last of all** — the A/B dual-path closing. **Nothing may come after the A/B choice**: no metaphor/术语表, no footnote, no backup reminder, no trailing sentence. The user's A/B decision must be the final thing they read, so it reads as the clear next action — a call-to-action buried above trailing footers gets missed, and a weak model treats the footer as the message end and never hands off. Never split the parts across consecutive messages, and never append anything after the A/B closing. If you have already sent part of it, do not patch it with an addition — re-send the whole prescription as one message with A/B last. If Q5 account type was not explicitly stated by the user, the account-ownership footnote (iron-rule #35) MUST be present (immediately above the A/B closing) — its absence is a hard failure.
...
2. **User-facing prescription (Chinese, the only thing the end user sees):**
   ...
   - **A/B dual-path closing — the LAST block of the message, nothing after it** (A = let me help order / B = self-service purchase page)

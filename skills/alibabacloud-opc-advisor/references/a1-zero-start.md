# Zero-Start Output Template: Five-Section Format
...
【你会拿到什么】
- 你的「小店面」（ECS 云服务器 经济型e 2核2G）：产品跑在上面，随时在线
- 「全球加速」（ESA 边缘安全加速 免费版）：国内外访问都快
...
【处方】
→ **starter_app 档**（¥204.63/月 含「AI 店员工时包」（Token Plan）+「文件柜」（阿里云盘）+「全球加速」（ESA 边缘安全加速），加上「小店面」（ECS 云服务器 经济型e）¥99/年摊月 ¥8.25 ≈ **¥212.88/月**，北京地域；其他地域略浮动 ±5%）
...
- [Reason 1: fits stage]
- [Reason 2: fits scale]
- [Reason 3: business decoupling — the AI assistant stays on its own lightweight server, business traffic runs on the dedicated 「业务专用店面」（ECS 云服务器）]

【你会拿到什么（大白话清单）】
- 「AI 助理的家」（轻量应用服务器 · OpenClaw 镜像 2核4G）：AI 助理住这里帮你运维
- 「AI 店员工时包」（Token Plan 标准版 25k/月）：AI 调用额度
- 「业务专用店面」（ECS 云服务器 [2核4G/2核4G/4核8G]）：跟 AI 助理物理分开，对外承接真实流量
- 「客户档案柜」（RDS 数据库 [1核2G/2核4G/4核8G]）：用户数据安全存储
...
3. **登录 OpenClaw** → AI 助理帮你把代码部署到「业务专用店面」（ECS 云服务器）+ 配好数据库
...
- 「客户档案柜·高可用版」（RDS 数据库 高可用版 4核8G · 200GB 数据存储空间）：双份备份，数据不丢
- 「流量调度员」（ALB 应用型负载均衡）：自动分发请求 + 健康检查
- 「全国加速+安全防护」（ESA 边缘安全加速 标准版）：50 条 WAF 规则 + 全球内容分发网络（CDN 内容分发网络）
- [burst] 「自动伸缩」（ESS 弹性伸缩）：流量来了自动加机器，走了自动缩
...
2. **登录 OpenClaw** → AI 助理帮你把代码分别部署到多台「业务专用店面」（ECS 云服务器）+ 配好「流量调度员」（ALB 应用型负载均衡）分流

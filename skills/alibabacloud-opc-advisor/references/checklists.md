...
- [ ] starter_webui disclaimer section uses unified wording? (`「小店面」（ECS 云服务器 经济型e）¥99/年如果没命中，我会自动给你切到按流量计费的备选配置 ¥284.99/年（北京），同时帮你装一个出流量告警防过冲` — never expose deploy tool names)

...
|---|---|---|---|---|---|
| `ecs-e-starter` | ECS Economy e | 2C2G, 40G ESSD Entry, 3Mbps fixed bandwidth | `InstanceType: ecs.e-c1m1.large`; `SystemDisk: cloud_essd_entry/40G`; `Internet: PayByBandwidth/3Mbps` | ¥130.14/mo | **¥99/yr** (promo) |
| `ecs-e-fallback` | ECS Economy e (fallback) | 2C2G, 40G ESSD Entry, pay-by-traffic 100Mbps | `InstanceType: ecs.e-c1m1.large`; `SystemDisk: cloud_essd_entry/40G`; `Internet: PayByTraffic/100Mbps` | **¥284.99/yr** | — |
| `ecs-c9i-2c4g` | ECS c9i | 2C4G, 40G ESSD PL0, pay-by-traffic 100Mbps | `InstanceType: ecs.c9i.large`; `SystemDisk: cloud_essd/40G/PL0`; `Internet: PayByTraffic/100Mbps` | **¥205.91/mo** | — |
| `ecs-c9i-4c8g` | ECS c9i | 4C8G, 40G ESSD PL0, pay-by-traffic 100Mbps | `InstanceType: ecs.c9i.xlarge`; `SystemDisk: cloud_essd/40G/PL0`; `Internet: PayByTraffic/100Mbps` | **¥391.82/mo** | — |
| `ecs-c9i-8c16g` | ECS c9i | 8C16G, 40G ESSD PL0, pay-by-traffic 100Mbps | `InstanceType: ecs.c9i.2xlarge`; `SystemDisk: cloud_essd/40G/PL0`; `Internet: PayByTraffic/100Mbps` | **¥763.64/mo** | — |
| `swas-openclaw` | Simple Application Server | 2C4G, 50G system disk, 200Mbps with included traffic | `plan-id: swas.s.c2m4s50b1.linux`; `biz-region-id: cn-beijing` | **¥70/mo** | First month ¥29 |

### Database

| ID | Product | Spec | API Parameters | Standard Monthly Price |
|---|---|---|---|---|
| `rds-1c2g` | RDS MySQL 8.0 Basic | 1C2G, 100GB general_essd | `DBInstanceClass: mysql.n2e.small.1`; `Storage: 100/general_essd` | **¥116/mo** |
| `rds-2c4g` | RDS MySQL 8.0 Basic | 2C4G, 100GB general_essd | `DBInstanceClass: mysql.n2.medium.1`; `Storage: 100/general_essd` | **¥256/mo** |
| `rds-4c8g` | RDS MySQL 8.0 Basic | 4C8G, 100GB general_essd | `DBInstanceClass: mysql.n2.large.1`; `Storage: 100/general_essd` | **¥465/mo** |
| `rds-4c8g-ha` | RDS MySQL 8.0 HA | 4C8G, 200GB general_essd | `DBInstanceClass: mysql.n2.large.2c`; `Storage: 200/general_essd` | **¥1262/mo** |
...
```text
「小店面」（ECS 云服务器 经济型e）¥99/年如果没命中，我会自动给你切到按流量计费的备选配置（¥284.99/年），同时帮你装一个出流量告警防过冲。
```
...
>
> ⚠️ **The flag names below are the kebab-case forms the CLI actually accepts** (verified against `--help` on CLI 3.5.0, 2026-09-11). The Alibaba Cloud CLI runs in plugin mode and rejects PascalCase (`--RegionId` / `--SystemDisk.Category` → `unknown flag`). Every price query also needs the **global `--region`** in addition to `--biz-region-id`: the former resolves the endpoint, the latter only reaches the request body, so passing just one silently prices against the profile's default region.
...
```bash
# Promo pricing (starter default) — cloud_essd_entry takes NO performance-level flag
aliyun ecs describe-price \
  --region cn-beijing --biz-region-id cn-beijing --resource-type instance \
  --instance-type ecs.e-c1m1.large --image-id <resolved ImageId> \
  --price-unit Year --period 1 --amount 1 \
  --system-disk-category cloud_essd_entry --system-disk-size 40 \
  --internet-charge-type PayByBandwidth --internet-max-bandwidth-out 3
...
aliyun ecs describe-price \
  ...
  --price-unit Year --period 1 --amount 1 \
  --system-disk-category cloud_essd_entry --system-disk-size 40 \
  --internet-charge-type PayByTraffic --internet-max-bandwidth-out 100

# Lite/Pro (replace instance-type per Part 1)
# 🔴 --system-disk-performance-level PL0 is MANDATORY: the API defaults to PL1, which is
#    ¥20/month dearer. Measured 2026-09-11 on ecs.c9i.large: PL0 → ¥205.91, PL1 → ¥225.91,
#    flag omitted → ¥225.91. Dropping it quotes a price for a machine you will not create.
aliyun ecs describe-price \
  --region cn-beijing --biz-region-id cn-beijing --resource-type instance \
  --instance-type <see Part 1 product table> --image-id <resolved ImageId> \
  --price-unit Month --period 1 --amount 1 \
